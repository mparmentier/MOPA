...
liste_CODE_DESCRIPTEUR = [88,12] # pour le choix de la categorie des documents ('ANNONCE.GESTION.INDEXATION.DESCRIPTEURS.DESCRIPTEUR.CODE')
NB_CLUSTERS = 5
MONGO_LIMIT = 280000
...
for i, CODE_DESCRIPTEUR in enumerate(liste_CODE_DESCRIPTEUR):
    print()
    ...
    clustersDB.insert_one({'CODE_DESCRIPTEUR' : CODE_DESCRIPTEUR,
                           'LIBELLE' : libel,
                           'nb_textes': len(texts)})
    ...
    for cluster_id in X.index:
        ...
        annonces.append(texts_of_this_cluster.values[0][0])
        
        for text in texts_of_this_cluster.values:
            annonces.append(text[0])
            with open(os.path.join(cluster_path,text[0]),'w') as fi:
                # J'ai toujours le problème de l'encodage
                # Je ne mets pas de texte dans le fichier. C'est visible sur le site internet...
                # Ligne d'origine : fi.write(text[1])
                fi.write('test')
    ...
    for row in data_meta.itertuples() :
        ...
        result = clustersDB.update_one(
            {"CODE_DESCRIPTEUR": CODE_DESCRIPTEUR},
            {
                "$set": { row[3]: {
                            "key_words": row[1],
